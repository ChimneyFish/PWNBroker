...
def build_pdf_report(scans) -> bytes:
    ...
    h2_style = ParagraphStyle("H2", parent=styles["Heading2"],
                               fontSize=14, textColor=colors.HexColor("#1e293b"), spaceBefore=16)
    ...
    for scan in scans:
        ...
        if not results:
            ...
            t = Table(table_data, colWidths=[0.7*inch, 1.8*inch, 1.4*inch, 0.6*inch, 2.5*inch])
    ...
    for scan in scans:
        ...
        for r in results:
            color = {"critical": "#dc2626", "high": "#ea580c", "medium": "#d97706",
                     "low": "#2563eb", "info": "#6b7280"}.get(r.severity, "#6b7280")
            rows.append(f"""
            <tr>
              <td>{scan.name}</td>
              <td>{scan.target.host}</td>
              <td><span style="color:{color};font-weight:bold">{r.severity.upper()}</span></td>
              <td>{r.title}</td>
              <td>{r.host or ''}</td>
              <td>{r.cvss_score or ''}</td>
              <td>{(r.description or '')[:120]}</td>
            </tr>""")
...
<table>
<thead><tr><th>Scan</th><th>Target</th><th>Severity</th><th>Title</th><th>Host</th><th>CVSS</th><th>Description</th></tr></thead>
