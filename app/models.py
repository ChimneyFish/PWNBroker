...
class SSOConfig(db.Model):
    ...
    # SAML 2.0 (e.g. Microsoft Entra Enterprise App "SAML-based sign-on") — a
    # separate mechanism from the OAuth/OIDC providers above. Shares the same
    # allowed_domains/auto_provision policy below rather than having its own,
    # so "who's allowed to SSO in" is governed in exactly one place.
    saml_enabled           = db.Column(db.Boolean, default=False)
    saml_sp_entity_id      = db.Column(db.String(512))   # optional override; defaults to this SP's own metadata URL
    saml_idp_entity_id     = db.Column(db.String(512))
    saml_idp_sso_url       = db.Column(db.String(512))
    saml_idp_x509_cert     = db.Column(db.Text)          # IdP signing cert — public, not a secret
    saml_metadata_xml      = db.Column(db.Text)          # raw uploaded/fetched metadata, kept for reference
