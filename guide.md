...
#### Step 2 — Note the registration token

The registration token is auto-generated. Find it on the **Threat Intel → Agents → Download Agent** page (`/threat/download`). All agents must present this token when registering for the first time. You do not need to set it manually — it is created automatically on first use.
...
API keys are stored in the database. Entering a key and saving replaces the existing value; leaving a field blank keeps the current key. Click **Test All Keys** to verify connectivity for each configured service.
...
```json
// Request body (auth via X-Agent-ID and X-Agent-Token headers)
{
  "ip_address": "10.0.0.45",
  "connections": [
    { "remote_ip": "185.220.101.5", "status": "ESTABLISHED", "pid": 1234 }
  ]
}
